...
Notice what the user never sees: the endpoint URL, the model, the credentials, the data. They just see a text box and an answer. That separation is the entire mental model.
...
It is a loop: *show the box, take the text, call the endpoint, show the reply.* That simplicity is exactly what makes it safe and quick to build.
...
➡️ [Agent Bricks: Custom LLM & Document Processing](/docs/building-agents/agent-bricks-idp)
