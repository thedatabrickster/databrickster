...
Take a breath. This is a code-heavy build lesson, and that is exactly what you signed up for. Nothing here is new magic. Every piece is something you have already met, just assembled into one coherent whole. We will move slowly, narrate every code block, and pause to celebrate each milestone. You are building a real agent today. You are ready.
...
Notice that MLflow tracing sits quietly around all of this. Because your agent follows the standard interface and uses the OpenAI-compatible client, MLflow can record every model call, every tool call, and every timing as a single trace you can open and inspect. You will turn that on with one line.
...
- Question 1 should trigger `search_policies`, and the answer should cite the policy document it read.
- Question 2 should trigger `get_client_summary` and report the client's risk profile as an exact fact.
- Question 3 is the exciting one: the model should call `get_client_summary` first, then `search_policies`, then combine both into a single grounded, cited answer. That is your agent reasoning across structured and unstructured data.
- Question 4 should produce "I don't know based on Northwind's records." Neither tool applies, and the rule holds. Seeing this is just as important as seeing the others succeed: it proves your guardrail works.
...
You built Northwind Trust's Advisor Assistant end to end. You defined two tools: `search_policies`, a Vector Search retrieval tool over unstructured policy documents that returns citable passages, and `get_client_summary`, a governed Unity Catalog SQL function that returns exact, structured client facts. You described both tools so function calling could choose between them. You implemented a `ResponsesAgent` whose `predict` runs the reason-act-observe loop, capped for safety, and composes a grounded, cited answer while enforcing the "say I don't know" rule through the system prompt. You turned on MLflow tracing with one line and tested locally on a policy question, a client-data question, a compound question that needed both tools, and an unanswerable question that correctly produced "I don't know." Everything you assembled here is a piece you had already learned; the win was wiring them into one coherent, trustworthy agent.
