...
- Describe the agent loop: reason, act, observe, repeat.
...
The magic word is **loop**. A chatbot answers once. An agent works in a loop, taking one step at a time, until the goal is met.

You don't need to memorize that yet. Just hold the picture: an assistant who keeps working until the task is finished.
...
That's the entire concept. Reason, act, observe, repeat. Everything else is detail.
...
An AI agent is a language model that can **decide and act in a loop** instead of just answering once. It follows a simple heartbeat: **reason, act, observe, repeat**, using **tools** (actions like querying a table or searching documents) that it chooses itself, until the task is done. This is different from a RAG pipeline, which walks one fixed path, retrieve then answer. Crucially, RAG can live *inside* an agent as one of its tools. We saw all of this through Northwind Trust, where an agent looked up a client's balance and searched policy docs to answer one compound question, choosing both steps on its own. You've now got the whole mental model. Nicely done.
