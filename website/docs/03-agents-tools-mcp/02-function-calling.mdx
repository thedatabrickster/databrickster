...
This is the labeled form. The `name` matches our Python function. The `description` tells the model when to reach for it. The `parameters` block is a JSON schema saying, "this tool needs one input, `account_id`, and it's a string, and it's required." The model reads all of this to decide whether and how to call the tool.
