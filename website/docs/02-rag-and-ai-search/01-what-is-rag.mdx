...
Notice how little the model itself changed. Steps 2, 3, and 4 are all *your* pipeline — embed, search, assemble. The model only shows up at step 5, and by then the hard thinking (finding the right facts) is already done. RAG is mostly good data engineering with a language model bolted on the end.
...
Read that list again and you will notice steps 1 through 5 are pure data work — ingest, transform, index, query. Only steps 6 and 7 touch the model.
...
- RAG turns a closed-book exam into an open-book one: most of it is a data pipeline — ingest, embed, index, search, assemble — and the model only shows up at the very end.
