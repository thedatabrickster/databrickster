...
And crucially, **each chunk carries a little luggage** — its metadata. For every chunk we remember which file it came from, which page, and its position in order. This is what lets the assistant later say "according to *Travel Policy 2026*, page 4…" instead of a naked, unattributable answer.
...
We started with a simple problem: Northwind Trust has long policy PDFs and wants an assistant that answers questions from them. You cannot embed a whole document as one vector — it will not fit the model's input limit, and its meaning smears into an unhelpful average. So we cut documents into **chunks**: small, self-contained passages, each embedded on its own.

You learned the three knobs that control chunking — **size** (in tokens), **overlap** (the repeated safety margin at the edges), and **strategy** (fixed, structure-based, or recursive) — and the trade-offs of each. Too small loses context; too big gets noisy and blurry. You saw why every chunk should carry **metadata** like source file and page. And you built the real thing: a token-based chunker with overlap, a recursive splitter, and a write to a Delta table with one row per chunk — exactly the artifact the next lesson will index.
