...
- **Model consistency (this is the big one).** A query embedding and the stored document embeddings **must come from the same model and same version**. Vectors from `databricks-gte-large-en` and `databricks-bge-large-en` live in *different* spaces and are not comparable — comparing them yields garbage similarities, silently. Pin the model name everywhere, store the model name/version in the table so every vector is self-describing, and treat a model upgrade as a full re-embed of your corpus. Design your pipeline so that re-embed is a routine, repeatable job, not a heroic effort.
