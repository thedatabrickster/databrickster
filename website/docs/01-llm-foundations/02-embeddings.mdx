...
> An embedding is the bridge between language and math: it turns fuzzy human meaning into coordinates on a map, so that ideas which *mean* the same thing land near each other — and a computer can finally measure how close two meanings are.
...
- Distinguish an **embedding model** (outputs a vector) from an **<abbr title="Large Language Model">LLM</abbr>** (outputs text), and explain what "dimensions" means.
...
**3. Similar things cluster.** All the "withdrawal / redemption / cash-out" phrasings from Northwind Trust's tickets land in one region. All the "onboarding / new account / <abbr title="Know Your Customer — the regulated identity-verification process for new clients">KYC</abbr>" phrasings land in another. If you projected the space down to 2D and plotted it, you'd see visible blobs by topic.

**4. Context matters (modern models are contextual).** Older embedding techniques (like `word2vec`) gave every word one fixed vector. Modern transformer-based models like <abbr title="General Text Embeddings — the model family behind databricks-gte-large-en">GTE</abbr> and <abbr title="BAAI General Embedding — from the Beijing Academy of Artificial Intelligence; behind databricks-bge-large-en">BGE</abbr> are **contextual**: the word "bank" gets a different vector in "river bank" than in "savings bank," because the model reads the whole sentence before producing the vector. This is a big deal for real business text where the same word means different things.

:::note[What GTE and BGE stand for]
**GTE** = *General Text Embeddings*, and **BGE** = *BAAI General Embedding* (from the Beijing Academy of Artificial Intelligence). Both are open, transformer-based embedding model families; Databricks serves them as `databricks-gte-large-en` and `databricks-bge-large-en`. Throughout this site, hover (or tap) any dotted-underlined acronym to see what it stands for.
:::
...
4. **Pool into one vector.** We now have one contextualized vector *per token*, but we want *one* vector for the whole input. The model **pools** them — commonly by taking a special <abbr title="Classification token — a special token whose final vector is often used to represent the whole sequence">`[CLS]`</abbr> token's vector, or by averaging all token vectors ("mean pooling"). The result is a single fixed-length vector.
...
### 4. Python — persist embeddings back to a Delta table with a Spark <abbr title="User-Defined Function — a custom function you register to run over DataFrame rows">UDF</abbr>
...
- **Idempotency & incrementality.** Embedding is expensive (a model call per row). Don't re-embed unchanged rows. Use a hash of the source text (`sha2(body, 256)`) as a change key; only embed rows whose text hash changed. This is classic incremental <abbr title="Extract, Transform, Load — the standard data-pipeline pattern">ETL</abbr> thinking applied to embeddings.
...
- **Embeddings can leak information.** A vector is derived from the source text and, while not human-readable, is *not* anonymization. Research shows text can sometimes be partially reconstructed from embeddings. Treat an embedding of sensitive data (<abbr title="Personally Identifiable Information">PII</abbr>, <abbr title="Protected Health Information">PHI</abbr>, <abbr title="Material Non-Public Information — price-sensitive information not yet public">MNPI</abbr>) with the **same** access controls as the source text. Apply Unity Catalog governance to the embedding column/table.
...
- **GTE (General Text Embeddings)**: An open transformer-based embedding model family; served on Databricks as `databricks-gte-large-en` (1024-dim).
- **BGE (BAAI General Embedding)**: An open transformer-based embedding model family from the Beijing Academy of Artificial Intelligence (BAAI); served on Databricks as `databricks-bge-large-en` (1024-dim).
- **LLM (Large Language Model)**: A generative model that outputs text (predicts the next tokens), as opposed to an embedding model that outputs a vector.
