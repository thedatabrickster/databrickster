...
Now we log (package) the agent and pass in the `resources` list. This is the moment the declaration "sticks" to the model, so that at deployment the scoped credentials cover exactly these resources and nothing else.
...
This one is intentionally conceptual, because the exact function names evolve. The idea you are expressing is a single, powerful sentence: *"For every request, run as the person who made it."* Choose this when users must be isolated; skip it and you fall back to the service-principal default the comment describes. Either way, be deliberate: the choice is a security decision, not a default to accept blindly.
...
An agent is only ever as powerful as the identity it acts as — that is the heart of everything here.
