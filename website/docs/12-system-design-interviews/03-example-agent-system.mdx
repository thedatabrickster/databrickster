...
**Tool design and schemas.** Each tool gets a narrow name, a plain description, and a typed schema, for example `open_support_case(account_id: string, reason: string, priority: enum)`. The narrow-beats-vague reasoning is in the Deep Dive above; here it means a concrete, auditable signature for every tool.

**Function calling.** The model emits a structured "call this tool" request and the orchestrator runs the real function (see Theory). That boundary is exactly where I insert checks and approval.
