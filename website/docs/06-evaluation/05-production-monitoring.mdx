...
And you have an even closer analogy from your day job: **a pipeline dashboard.** You already stare at freshness, failure rates, and row counts. Production monitoring is that same dashboard, with quality scores added as new "vital signs."
...
Remember: every time your deployed app answers a request, it produces a **trace**, the itemized receipt of what happened (the question, what the retriever pulled, the exact prompt, the model's answer, the latency, the token count). In production, those traces are captured automatically and stored.
...
Evaluation doesn't stop at launch. Offline evaluation is a gate: a one-time check on a fixed dataset that decides whether a version ships. Online monitoring is a continuous watch: it runs the *same* scorers and judges on a *sample* of live traffic, captured as traces, and tracks quality, latency, and cost on dashboards over time. This lets you catch drift, get alerted when a metric slips, trace the cause, and feed real-world failures back into your evaluation dataset. It's the same discipline you already apply to your data pipelines, pointed at AI quality.
