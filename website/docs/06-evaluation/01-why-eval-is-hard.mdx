...
Reading this top to bottom: we define a small dataset where each row carries a question, its source context, and an ideal answer. We loop over every row, ask the AI, and grade the answer against the rubric. We collect all the scores and then `summarize()` averages them into one report. That final `report` dictionary is the thing you show your boss and your regulator. It is the "it answers correctly 94% of the time" sentence, made of code.
