...
Notice how the second picture instantly answers the question the first one could not. That is not magic. It is just the same "look inside before you guess" habit you already use with logs and the Spark UI, applied to an agent.
...
An AI agent is not like a SQL query. A query is deterministic and basically one step, so it is easy to reason about. An agent is nondeterministic and takes many hidden steps — decide, retrieve, call a tool, call the model, decide again — and from the outside you only see the question and the answer. When the answer is wrong, you cannot fix it unless you can see what happened at each step. **Observability** is making the inside of a run visible. It gives you a **trace** (the whole run) made of **spans** (the individual steps, each with inputs, outputs, and timing) — an airplane black box and an itemized receipt for your agent. Northwind's team went from arguing in the dark for hours to spotting a wrong retrieval in fifty milliseconds. And the best part: this is the same instinct you already use with logs, query plans, and Spark UI stages, and turning it on is essentially one setting.
