...
The code below does both. A note on tools: `tiktoken` is the tokenizer used by several popular models and is perfect for *illustration* because it is tiny and instant. The exact token boundaries and IDs differ from one model family to another, so treat the numbers as representative, not as the ground truth for the specific Databricks-hosted model you will bill against. The **concept** transfers exactly; the **counts** are approximate across model families.
...
- **Input tokens cost time too**, but they are processed together up front (the "prefill"), so a long prompt mainly adds a fixed startup delay; long *outputs* dominate end-to-end latency.
- **Fewer tokens = faster and cheaper, simultaneously.** Concision is not just a cost tactic; it is a latency tactic. Tight prompts and bounded outputs win on both axes.
...
A **token** is the sub-word chunk that an LLM actually reads and generates — the model's equivalent of a record, the granular unit text is split into before processing. Models use sub-word tokens (learned via **BPE**) because whole-word vocabularies are infinite and break on unseen words, while single characters are hopelessly inefficient; sub-words give compact common words and fragmentable rare words with no out-of-vocabulary failures. Text is **encoded** into token IDs, the model operates only on those IDs, and the IDs are **decoded** back to text in a deterministic, lossless round-trip. As a rule of thumb, **1 token ≈ 0.75 English words ≈ 4 characters**, inflated by punctuation, formatting, numbers, code, and non-English text. Most importantly, the token is the unit of **two** hard limits: **billing** (Databricks Foundation Model APIs charge per input + output token — the LLM's DBU) and the **context window** (a fixed token budget shared by prompt and answer). Reasoning in tokens instead of documents is the foundation for every cost, latency, and quality decision you'll make from here.
