...
A prompt controls behavior as directly as code does, so it deserves version control, review, evaluation, and rollback. The MLflow Prompt Registry gives you exactly that by storing prompts as versioned, governed assets in Unity Catalog. You register a prompt to get immutable versions, attach a movable alias like `production`, and load the prompt in your app by `name@alias`. Promoting a new prompt is just moving the alias forward; rolling back is moving it back - both without a code deploy, and both backed by full history and audit. Evaluation against your Part 6 dataset turns promotion into a data-driven decision, and auto-optimization can even draft better versions for you to test.
