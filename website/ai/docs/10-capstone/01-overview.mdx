...
Notice something reassuring: there is nothing in that list you have not met before. The capstone is a rearrangement of ideas you already own.
...
*Diagram: Every capstone component traces back to a Part. Reading left to right, each Part of the course on the left produced a specific capability on the right that the Advisor Assistant now uses. Nothing in the capstone is new material.*
...
The capstone is the **Advisor Assistant**: an internal agent for Northwind Trust's financial advisors that answers two kinds of questions — fuzzy policy questions from **documents** (via a RAG retrieval tool) and precise account-fact questions from **governed tables** (via a structured tool) — and reasons over both in an **agent loop**. Around that agent sits an operational wrapper: **tracing** to observe it, **evaluation** to score it, **serving** to expose it, **CI/CD** to promote changes safely, and **governance** to enforce safety, per-user permissions, and cost.

The most important thing to feel right now: **none of this is new.** Every box on the architecture maps to a Part you already studied. This lesson was the blueprint. The next lesson builds the agent, and the one after that evaluates, deploys, and governs it. You have everything you need.
