...
This is worth calling out on its own because it is so useful. Your agent has already run, in development or in production, and each run left a trace. You can take those traces and ask experts to label them after the fact. You are not making up test questions, you are grading *real* traffic. That keeps your evaluation grounded in what users actually ask.
...
*Figure 1: The virtuous feedback loop. Agent outputs are graded by humans in the Review App and by end users. Those human labels do double duty: they enrich your evaluation datasets and they calibrate your LLM judges, which makes your automated evaluation trustworthy enough to run at scale, on the next batch of agent outputs.*

The single most important idea in this whole lesson lives in that diagram: human labels are not just grades, they are the fuel that makes automated grading believable.
...
Automated judges are wonderful for scale, but they are only trustworthy if humans have checked their work. On Databricks, MLflow 3 for GenAI lets you collect human feedback three ways: experts review and label in the **Review App**, you can **label existing traces** from real traffic, and you can gather **end-user thumbs-up/down** from a deployed app. Every grade is stored as an assessment (feedback or expectation) attached to a trace. Those human labels do double duty: they enrich your evaluation datasets and they calibrate your LLM judges. That calibration is what lets you scale evaluation without a human on every answer. And for high-stakes, nuanced correctness, like a compliance officer confirming a policy answer, humans remain irreplaceable.
