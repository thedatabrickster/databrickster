...
chat UI in front of an agent — mostly by describing what you want.
...
A focused, three-page deep dive. Go in order the first time.

```mermaid
flowchart TD
    L1["1 · This page<br/>what Claude Code is, why it fits"]
    L2["2 · Setup<br/>Claude Code + the AI Dev Kit (MCP + skills)"]
    L3["3 · Build a Databricks App<br/>end-to-end, hands-on flagship"]

    L1 --> L2 --> L3
```

*The Claude Code subtopic. Each page uses the same running example — Maya, an engineer at
"Northwind Trust," shipping a governed chat App on Databricks with Claude Code as her pair.*

## Pages

1. **This overview** — what Claude Code is, the agentic loop, and why it pairs so well with Databricks work.
2. **[Setup: Claude Code + the Databricks AI Dev Kit](/agentic-coding/claude-code/setup-ai-dev-kit)** — install Claude Code, install the AI Dev Kit, and connect the MCP server and skills so the assistant can use governed Databricks tools.
3. **[Build a Databricks App with Claude Code](/agentic-coding/claude-code/build-a-databricks-app)** — the flagship: go from an empty folder to a deployed, governed chat App in front of an agent, driving Claude Code step by step.
...
Ready? Start with **[Setup: Claude Code + the Databricks AI Dev Kit](/agentic-coding/claude-code/setup-ai-dev-kit)**.
